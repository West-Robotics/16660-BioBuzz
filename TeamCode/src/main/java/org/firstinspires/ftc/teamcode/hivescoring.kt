...
/**
 * Wraps the pollen/nectar ColorBlobLocatorProcessors so a single instance
 * feeds the FRONT portal AND provides a "nearest ball" query for the capture
 * assist. Distance/angle use the same pinhole math as the old VisionSystem
 * (C270_FOCAL_LENGTH_PIXELS at 640x480 + known physical ball widths).
 */
class BallLocator {

    private val pollen = locator(POLLEN_HSV_RANGE)
    private val nectarBlue = locator(NECTAR_BLUE_HSV_RANGE)
    private val nectarRed = locator(NECTAR_RED_HSV_RANGE)

    /** Add these to the FRONT camera portal. */
    val processors = listOf(pollen, nectarBlue, nectarRed)

    private fun locator(range: org.firstinspires.ftc.vision.opencv.ColorRange) =
    ...
    /** Closest visible ball across all colors, or null if only noise is visible. */
    fun nearestBall(): BallTarget? {
        var best: BallTarget? = null

        val sources = listOf(
            Triple(pollen, "Pollen", POLLEN_PHYSICAL_WIDTH_MM),
            Triple(nectarBlue, "Nectar Blue", NECTAR_PHYSICAL_WIDTH_MM),
            Triple(nectarRed, "Nectar Red", NECTAR_PHYSICAL_WIDTH_MM),
        )
        for ((proc, label, widthMm) in sources) {
            val blobs = try {
                proc.blobs.toList()
            } catch (_: Exception) {
                emptyList()
            ...
            for (blob in blobs) {
                ...
                val distanceCm = C270_FOCAL_LENGTH_PIXELS * widthMm / pixelWidth / 10.0
                val bearingDeg = Math.toDegrees(
                    atan2(box.center.x - GROUND_CAM_WIDTH / 2.0, C270_FOCAL_LENGTH_PIXELS),
                )
                val candidate = BallTarget(
                    label = label,
                    distanceCm = distanceCm,
                    bearingDeg = bearingDeg,
                ...
                if (best == null || candidate.distanceCm < best.distanceCm) best = candidate
            }
        }
        return best
    }
